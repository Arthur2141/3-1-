...
PhongDrawShadowShader::PhongDrawShadowShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PhongDrawShadowVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PhongDrawShadowPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void PhongDrawShadowShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
...
// 描画
void PhongDrawShadowShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void PhongDrawShadowShader::End(ID3D11DeviceContext* dc)
{
