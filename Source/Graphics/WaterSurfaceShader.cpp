...
WaterSurfaceShader::WaterSurfaceShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\WaterSurfaceVS2.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\WaterSurfacePS2.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void WaterSurfaceShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	cbScene.reflectViewProjection = rc.reflectViewProjection;//追加

	dc->UpdateSubresource(sceneConstantBuffer.Get(), 0, 0, &cbScene, 0, 0);
}

// 描画
void WaterSurfaceShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void WaterSurfaceShader::End(ID3D11DeviceContext* dc)
{
