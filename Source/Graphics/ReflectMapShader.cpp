...
ReflectMapShader::ReflectMapShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ReflectMapVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ReflectMapPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void ReflectMapShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	cbScene.param.x = rc.water_surface_height;
	dc->UpdateSubresource(sceneConstantBuffer.Get(), 0, 0, &cbScene, 0, 0);
}

// 描画
void ReflectMapShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void ReflectMapShader::End(ID3D11DeviceContext* dc)
{
