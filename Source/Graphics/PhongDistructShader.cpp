...
PhongDistructShader::PhongDistructShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PhongDistructVS.cso", "rb");
	...
	// ジオメトリシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PhongDistructGS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PhongDistructPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void PhongDistructShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	dc->VSSetShader(vertexShader.Get(), nullptr, 0);
	dc->PSSetShader(pixelShader.Get(), nullptr, 0);
	dc->GSSetShader(geometryShader.Get(), nullptr, 0);//追加
	...
	dc->GSSetConstantBuffers(0, ARRAYSIZE(constantBuffers), constantBuffers);//追加
...
// 描画
void PhongDistructShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void PhongDistructShader::End(ID3D11DeviceContext* dc)
{
	dc->VSSetShader(nullptr, nullptr, 0);
	dc->PSSetShader(nullptr, nullptr, 0);
	dc->GSSetShader(nullptr, nullptr, 0);//追加
