...
UseCubeMapShader::UseCubeMapShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\UseCubeMapShaderVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\UseCubeMapShaderPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void UseCubeMapShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	// CubeTexture
	dc->PSSetShaderResources(2, 1, &rc.cubeMapShaderResourceView);
...
// 描画
void UseCubeMapShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void UseCubeMapShader::End(ID3D11DeviceContext* dc)
{
