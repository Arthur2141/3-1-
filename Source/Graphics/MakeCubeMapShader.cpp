...
MakeCubeMapShader::MakeCubeMapShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\makeCubeMapShaderVS.cso", "rb");
	...
	// ジオメトリシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\makeCubeMapShaderGS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\makeCubeMapShaderPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	//// テクスチャの生成
	//char* filename = "Data/Shader/ramp3.png";
	//if (filename != nullptr)
	//{
	//	// マルチバイト文字からワイド文字へ変換
...
// 描画開始
void MakeCubeMapShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	//// RampTexture
	//dc->PSSetShaderResources(1, 1, shaderResourceView.GetAddressOf());
	//// ShadowMap
	//dc->PSSetShaderResources(2, 1, &rc.shaderResourceView);
...
// 描画
void MakeCubeMapShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void MakeCubeMapShader::End(ID3D11DeviceContext* dc)
{
