...
NoLightShader::NoLightShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\NoLightVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\NoLightPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void NoLightShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
...
// 描画
void NoLightShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void NoLightShader::End(ID3D11DeviceContext* dc)
{
