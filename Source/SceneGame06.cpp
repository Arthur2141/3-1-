...
// 更新処理
void SceneGame06::Update(float elapsedTime)
{
	// カメラコントローラー更新処理
...
void SceneGame06::Render()
{
	...
	{
		//　トゥーンシェーダ
		Shader* shader = graphics.GetShader(static_cast<int>(ShaderType::PhongShader));
		shader->Begin(dc, rc);
	...
	{
		//　トゥーンシェーダ
		Shader* shader = graphics.GetShader(static_cast<int>(ShaderType::PhongDistructShader));
		shader->Begin(dc, rc);
...
// エネミーHPゲージ描画
void SceneGame06::RenderEnemyGauge(
